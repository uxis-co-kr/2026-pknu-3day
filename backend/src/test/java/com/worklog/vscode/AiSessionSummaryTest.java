...
class AiSessionSummaryTest {
    ...
    void convertsLegacyPrompts() {
        AiSessionSummary legacy = new AiSessionSummary(
                "sess-old", null, FIRST, LAST, 7, null, List.of("출석 중복 검증 로직 봐 줘", "테스트도 붙여 줘"), null);
    ...
    void buildsTitleFromFirstPrompt() {
        AiSessionSummary noTitle = new AiSessionSummary(
                "s", "  ", FIRST, LAST, 1, List.of(new AiTurn(FIRST, "출석 중복 검증 로직 봐 줘", null)), null, null);
        ...
        AiSessionSummary clipped = new AiSessionSummary(
                "s", null, FIRST, LAST, 1, List.of(new AiTurn(FIRST, longPrompt, null)), null, null);
        assertThat(clipped.title()).isEqualTo("가".repeat(40) + "…");

        AiSessionSummary empty = new AiSessionSummary("s", null, FIRST, LAST, 0, List.of(), null, null);
    ...
    void keepsRecordedTitle() {
        AiSessionSummary s = new AiSessionSummary(
                ...
                List.of(new AiTurn(FIRST, "푸시하고 백로그2 진행해", "백로그2 의 1순위를 끝냈습니다")), null, null);
