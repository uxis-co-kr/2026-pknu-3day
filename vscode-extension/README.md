...
| 계획 메모 | `WorkLog: 계획 작성` 으로 연 문서에 적은 줄들 |
...
- 뷰 제목 오른쪽에 **전송 · 계획 작성 · 새로고침** 버튼이 있다.
- 파일을 저장하면 1초 뒤 자동으로 갱신된다. diff 본문은 화면에 쓰지 않으므로 받아 오지 않는다.
- **AI 대화**에는 오늘 질문이 없는 대화도 `보내지 않음` 으로 함께 보인다. Claude Code
  사이드바에는 있는데 여기에 없으면 빠진 것처럼 보이기 때문이다. 질문이 하나도 없는
  대화는 넣지 않는다.
...
명령 팔레트 → **`WorkLog: 계획 작성`** (또는 사이드바 제목의 메모 아이콘) 을 누르면
가운데에 **계획 문서 탭**이 열린다. 창 맨 위의 한 줄 입력이 아니라 보통 편집기라
...
- **저장할 때** 반영된다. 상태바에 "계획 N건을 저장했습니다" 가 잠깐 뜬다.
  읽을 줄이 없으면 "계획으로 읽을 줄이 없어 오늘 계획을 비웠습니다" 라고 적는다 —
  아무 말이 없으면 저장이 안 먹은 것과 구별되지 않는다
- **문서가 곧 그날 계획 전부다.** 줄을 지우고 저장하면 그 계획도 없어진다
- 목록 표식(`-`, `*`, `1.`, `[ ]`)은 붙여도 되고 안 붙여도 된다 — 떼고 보낸다
...
| 명령 팔레트 → `WorkLog: 계획 작성` | 계획 문서 탭을 연다. 저장하면 반영된다 |
