...
설치 후 **대시보드에서 연결한다.** 확장에 옮겨 적을 것은 없다 — 주소도 키도 대시보드가 안다.

1. 대시보드 → 설정 → API 연동 → **새 키 발급**
2. 발급 창의 **`VS Code 에 연결`** (Cursor 는 그 옆 링크)
3. 편집기가 뜨면서 주소와 키가 저장되고, 한 번 보내 연결까지 확인한다

링크가 안 열리면(원격 접속·브라우저가 막음) 같은 창의 **`연결 코드 복사`** → 편집기 명령
팔레트 → **`WorkLog: 대시보드 연결`** 에 붙여 넣는다. **키는 각자 것이고, 주소는 같다.**

> 대시보드는 백엔드 주소를 `현재 호스트:8080` 으로 짐작한다 (`/api` 프록시가 같은 PC 의
> 8080 을 본다는 전제다). 배포가 다르면 `frontend/.env` 에 `VITE_BACKEND_PUBLIC_URL` 을 박는다.
...
| 확장이 전송에 실패 | 사이드바 둘째 줄의 주소가 자기 PC 를 가리키고 있다. 대시보드에서 **VS Code 에 연결**을 다시 누르거나, 그 줄을 눌러 서버 PC 주소로 바꾼다 |
| `VS Code 에 연결` 을 눌러도 아무 일이 없다 | 브라우저가 편집기 호출을 막았거나 원격으로 붙어 있다. **연결 코드 복사** → 명령 팔레트 `WorkLog: 대시보드 연결` |
| 연결했는데 `서버에 연결할 수 없음` | 대시보드가 짐작한 주소(`호스트:8080`)가 틀렸다. `VITE_BACKEND_PUBLIC_URL` 을 박거나 사이드바 둘째 줄에서 고친다 |
