...
function TodayEditor({ draftId, onRegenerate, busy }: {
  ...
  async function onConfirm() {
    await confirm.mutateAsync({ id: draftId })
    setFlash('완료로 표시했습니다')
  }
      ...
      <div className="flex shrink-0 items-center justify-between gap-4 border-b px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="text-[12px] text-muted-foreground">
            ...
            {done && detail.data.confirmedAt
              ? `완료 ${formatTime(detail.data.confirmedAt)}`
              : dirty
                ? '저장하지 않은 변경'
                : `마지막 저장 ${formatTime(detail.data.updatedAt)}`}
          </span>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Button variant="outline" size="sm" className="h-8 gap-1.5" disabled={busy || done}
