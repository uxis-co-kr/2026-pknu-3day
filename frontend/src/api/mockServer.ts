...
const routes: [string, string, Handler][] = [
  ...
  ['POST', '/drafts/blank', (_p, q) => {
    const date = q.get('date') ?? ''
    const mine = db.details.find((d) => d.userId === db.me.id && d.workDate === date)
    if (mine) return mine
    const detail: Draft = {
      id: ++nextId, userId: db.me.id, workDate: date, version: 1, status: 'DRAFT',
      autoGenerated: false, userEdited: true,
      kind: 'DAILY' as const, periodStart: null, periodEnd: null, repoId: null, repoFullName: null,
      contentMd: `# ${date} 업무 일지 — ${db.me.name ?? db.me.login}\n\n## 완료한 작업\n- \n\n## 진행 중 / 미커밋\n- \n\n## 계획 / TODO\n- \n\n## 메모\n`,
      sourceActivities: [], sourceSessions: [],
      createdAt: now(), updatedAt: now(), confirmedAt: null,
    }
    db.details.push(detail)
    db.drafts.push({
      id: detail.id, userId: detail.userId, workDate: date, version: 1, status: 'DRAFT',
      autoGenerated: false, userEdited: true,
      kind: 'DAILY' as const, periodStart: null, periodEnd: null, repoId: null, repoFullName: null,
      createdAt: detail.createdAt, updatedAt: detail.updatedAt, confirmedAt: null,
    })
    return detail
  }],
